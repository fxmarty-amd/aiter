...
def test_topk_gpt_oss_shape_valid_expert_ids():
    ...
    arange = torch.arange(n_gates, device="cuda", dtype=torch.int64)

    print("expt_indx (padded row):", expt_indx[n_tokens_unpadded:].tolist())
    print("expt_hist.sum():", int(routing_data.expt_hist.sum()), "expected", n_gates)
    print("scatter_idx:", scatter_idx.to(torch.int64).tolist())

    # 1. The defect itself. No uninitialised memory involved, so this is
    #    deterministic on every aiter version.
    assert torch.all(expt_indx >= 0)
    assert torch.all(expt_indx < n_expts_tot)

    # 2. hist is a vertical popcount over the per-token expert bitmask, which is
    #    n_cols_pad (128) columns wide, but only [:n_expts_tot] is kept. Gates
    #    that elected a padded column are therefore dropped from the histogram
    #    while the counting sort still emits one gate per slot.
    assert torch.equal(
        routing_data.expt_hist.sum(), torch.tensor(n_gates, device="cuda")
    )

    # 3. The undercount collapses the tail of the gate array: every padded-column
    #    gate resolves to the same `gates` slot, so scatter_idx has duplicates and
    #    holes. Also deterministic -- ScatterIndx is fully written, unlike
    #    GatherIndx, so this does not depend on what torch.empty handed back.
    assert torch.equal(torch.sort(scatter_idx.to(torch.int64)).values, arange)

    # 4. Best effort only: the slots the sort never reaches keep whatever
    #    torch.empty returned, so this one depends on the allocator.
    assert torch.equal(torch.sort(gather_idx.to(torch.int64)).values, arange)


def _poison_free_blocks(device="cuda"):
    """Fill the caching allocator's free blocks with a large sentinel.

    Two buffers in the routing path are torch.empty and are read before being
    fully written: token_offs_raw past the n_expts_tot + 1 entries stage 1
    filled, and the gate slots the counting sort never reaches. Whether a bad
    expert id turns into a wild pointer or into silent in-bounds corruption
    depends entirely on what the allocator hands back. A long-running server
    runs on dirty memory; a fresh pytest process gets zeroed pages, which is
    why the assertion-only test above can pass without faulting.
    """
    blocks = []
    for numel in (16, 32, 64, 128, 256, 512, 1024, 4096, 1 << 16, 1 << 20):
        blocks.append(
            torch.full((numel,), 0x7FFFFFFF, dtype=torch.int32, device=device)
        )
    del blocks
    torch.cuda.synchronize()


def test_topk_gpt_oss_padded_logits_memory_fault():
    """Reproduce the memory access fault, not just the bad expert ids.

    Aborts the process with a HIP memory access fault. Run it on its own:
        pytest -q -s op_tests/triton_tests/moe/test_moe_routing_new.py \
            -k padded_logits_memory_fault

    Where it faults depends on the aiter version:

    * >= 73f149082 ("Moe a8w4 tp4 tuning", #4198) -- inside routing, in
      _routing_compute_indx_fused. That commit changed
      _compute_expt_data_internal from BLOCK = 128 to
      BLOCK = next_power_of_2(n_expts_tot), which desynchronised the
      token_offs_combined padding from topk's BLOCK_N = 128. TokensStart is now
      only written up to n_expts_tot, so an expert id of 126/127 reads the
      uninitialised tail and `gates` becomes the sentinel.

    * v0.1.16.post5 and earlier -- routing survives, because BLOCK = 128 matched
      topk's BLOCK_N and the unmasked store in the EQUAL_BLOCK=False branch of
      _expt_data_compute_stage1 covered TokensStart[0:128] with sum(hist). The
      fault moves to stage 1 instead: the counting sort still leaves gate slots
      unwritten, so GatherIndx holds the sentinel and the GEMM computes
      `X + (GatherIndx[i] // n_expts_act) * hidden_dim`.
    """
    if get_arch() not in ["gfx950", "gfx1250"]:
        pytest.skip("MOE stack not fully implemented on non-CDNA4 arch yet.")
    ...
    n_tokens = 8
    n_tokens_unpadded = 7
    # Divisible by 256 so the mxfp4 K blocking is happy, and large enough that
    # a bad gather index scaled by stride_x_m leaves the mapped region.
    hidden_dim = 2048
    ...
    _poison_free_blocks(device)

    routing_data, gather_idx, scatter_idx = routing(
        logits, n_expts_act, sm_first=False
    )
    torch.cuda.synchronize()
    print("survived routing; gather_idx:", gather_idx.to(torch.int64).tolist())
    ...
    # Only reached once the top-k fix is in: the padded row is allowed to be
    # non-finite, the unpadded rows are not. This is the vLLM
    # "AITER W4A8 stage2 output non-finite" assert.
